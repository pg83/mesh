...
def test():
    ...
    with lib.Lab(['a', 'b'], {1: ['a', 'b']}) as lab:
        ...
        bad(lambda c: c['registry'][0].update(index=0), 'index 0 is outside 1..255')
        bad(lambda c: c['registry'][1].update(index=0), 'index 0 is outside 1..255')
        bad(lambda c: c['registry'][0].pop('index'), 'index 0 is outside 1..255')
        bad(lambda c: c['registry'][1].update(index=256), 'index 256 is outside 1..255')
