...
def test():
    with lib.Lab(['a', 'b'], {1: ['a', 'b']}) as lab:
        ...
        assert reverse == reverse_before, (reverse, reverse_before)
        graph = lab.status('b')['graph']
        assert any(e['from']['addr'] == '10.1.0.101' and e['to'] == lib.endpoint('10.1.0.2') for e in graph)
        assert not any(e['from']['addr'] == '10.1.0.1' and e['to'] == lib.endpoint('10.1.0.2') for e in graph)
        stream.finish()
