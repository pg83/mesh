...
for test_path in build.glob("$(S)/tst/test_*.py"):
    ...
    e2e_tests.append(command(
        ...
        outputs=outputs,
        deps=[mesh, probe] if test_name in ("protocol", "gossip_stale_after_expiry", "graph_exchange", "route_attachment") else [mesh, quic] if test_name.startswith("quic") else [mesh],
        cmd=[
            *prelude,
            ["python3", test_path],
            touch(test_stamp),
