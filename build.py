...
for test_path in build.glob("$(S)/tst/test_*.py"):
    ...
    e2e_tests.append(command(
        ...
        outputs=outputs,
        deps=[mesh, probe] if test_name in ("echo", "protocol", "gossip_binary", "gossip_split", "registry_binary", "gossip_stale", "graph_exchange", "route_attachment", "ws_proxy", "ws_protocol", "ws_outgoing", "ws_loopback", "ws_channels", "route_payload") else [mesh, quic] if test_name.startswith("quic") else [mesh],
        cmd=[
            *prelude,
            ["python3", test_path],
            touch(test_stamp),
